<?php
class test_apiv4 extends APIWebTestCase
{
    //TODO:增加托管包以独立包的方式请求自己的公共资源的case
    public function __construct() {
        parent::__construct('apiv4');
    ...
    public function test_channel_sdk_info() {
        $uri = '/capi/apiv4/channel_sdk_info';
        $chn = 111111;
        $query = array(
            'chn' => $chn
        );
        ...
        foreach($data as $record) {
            ...
            //包名不能为空
            $this->assertFalse(preg_match('/"package_name":""/', $response));
            switch ($mode) {
                case 0:
    ...
    public function test_chngmlist() {
        $uri = '/capi/apiv4/channelgamelist';
        $requestparams = array(
            'chn' => 111111,
            'start' => 'abc',
            'len' => '0',
    ...
    public function test_rtcomp() {
        $uri = '/capi/apiv4/rtcomp';
        $requestparams = array(
            'ver' => '2.0.0.0',
        );
        $response = $this->send_request($uri, $requestparams, 'get');
    ...
    public function test_gamesodir() {
        // TODO : implement this
        $uri = '/capi/apiv4/gamesodir';
        $sql = "select revision.id,game.game_key, revision.hot_versioncode, so.channel_id, so.arch_type from cp_game_info game,cp_game_revision_info revision, cp_game_revision_apk_so so where revision.id=so.revision_id and game.game_id=revision.game_id";
        $db = $this->load->database("", TRUE);
        $query = $db->query($sql);
        foreach ($query->result_array() as $item) {
            $requestparams = array(
                'ver' => $item['hot_versioncode'],
                'gamekey' => $item['game_key'],
                'chn' => $item['channel_id'],
                'engine_arch' => $item['arch_type'],
            );
    ...
    public function test_channelsdkplug(){
        $uri = '/capi/apiv4/channelsdkplug';
        $this->setMaximumRedirects(0);
        $sdk = $this->db->select('file.id,chn.channel_id,file.download_url')->from('cp_third_channel_sdk chn_sdk')
                        ->join('cp_third_sdk file','file.third_channel_sdk_id = chn_sdk.id')
                        ->join('cp_channel_info chn','chn.third_channel_sdk_id = chn_sdk.id')
                        ->get()->result_array();
        $plugin = $this->db->select('file.id,chn.channel_id,file.download_url')->from('cp_third_channel_sdk chn_sdk')
                        ->join('cp_third_plugin file','file.third_channel_sdk_id = chn_sdk.id')
                        ->join('cp_channel_info chn','chn.third_channel_sdk_id = chn_sdk.id')
                        ->get()->result_array();
