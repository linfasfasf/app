...
<td><span class="main"><input class="input_txt" type="text" name="" maxlength="30" value="<?php echo $mapping['channel_id'].'/'.$mapping['channel_name']; ?>" disabled="disabled" /></span>
                        <span class="input input-xlarge uneditable-input" ><a href="<?php echo $mapping['apk_download_url'];?>"><?php echo basename($mapping['apk_download_url']);?></a></span></td>
                        <td>
...
-->
                            ...
                            <?php if(empty($mapping['apk_download_url']) || $mapping['active'] == 0){?>
                            <span class="dropdown">
                              <button class="btn btn-default dropdown-toggle" type="button" id="dropdownMenu1" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">
                                复制其他渠道
                                <span class="caret"></span>
                                  ...
                                  <?php 
                                        ...
                                        foreach ($mappings as $download) {
                                          if(!empty($download['apk_download_url']) && strcmp($download['channel_id'], $mapping['channel_id']))
                                          {
                                              ...
                                              ?>
                                  <li><a href="<?php echo site_url("game_management/experiment/copy_chnres?from={$download['apk_download_url']}&to={$mapping['channel_id']}&delete=$delete&chnres={$mapping['chnres_id']}&revid={$revision_id}");?>"><?php echo $download['channel_id'];?></a></li>
                                          <?php }
                                      }?>
                                  <?php if($no_option)
                                    {?>
                                        <li class="disabled"><a>请上传apk</a></li>
                                    <?php } ?>
                              </ul>
                            </span>
                            <?php } ?>
                        ...
                        <tr class="hide">
                            ...
                            <ul class="thumbnails center">
                              ...
                              <li class="span4">
                                  ...
                                  <?php
                                    $emp = TRUE;
                                    foreach ($arches as $key => $value) {
                                        if($value['channel_id'] == $mapping['channel_id'])
                                        {
                                            $emp = FALSE;
                                            echo $value['arch_type'].' ';
                                        }
                                    }
                                    if($emp)
                                    {
